...
## 2025-02-23 - [Batch Operation Estimated Time]
**Learning:** Batch processes that take a long time benefit significantly from providing an estimated time remaining. Users often experience "wait anxiety" when they only see time elapsed, but an explicit time remaining column sets clear expectations.
**Action:** When using `rich.progress` for long-running batch operations, always include `TimeRemainingColumn` alongside `TimeElapsedColumn`.
