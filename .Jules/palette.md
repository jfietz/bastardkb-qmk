...
**Learning:** Long-running batch operations (like compiling multiple firmwares) without an estimated completion time create wait anxiety for the user. Adding a `TimeRemainingColumn` to the `rich.progress` bar alongside elapsed time significantly improves the cognitive experience by setting clear expectations.
**Action:** Whenever implementing a progress bar for a batch operation with a known total count, always include an estimated time remaining indicator.
