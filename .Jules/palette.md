...
## 2024-05-20 - Add Estimated Time Remaining for Batch Builds
**Learning:** For long-running batch operations or builds, only showing time elapsed can cause wait anxiety for users. Providing a time remaining estimate is critical for reducing cognitive load and improving the perceived performance of the CLI.
**Action:** When using `rich.progress` for large tasks, always include `TimeRemainingColumn` alongside `TimeElapsedColumn` to give users an estimate of completion time.
