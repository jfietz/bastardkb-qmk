...
## 2024-05-29 - [Progress Bar Estimations]
**Learning:** Long-running batch operations that only show elapsed time create wait anxiety because users cannot gauge when the task will complete. Including a time remaining estimation greatly improves the perception of performance and progress.
**Action:** Whenever using `rich.progress` for batch or lengthy operations, always pair `TimeElapsedColumn` with `TimeRemainingColumn` to provide a complete time context.
