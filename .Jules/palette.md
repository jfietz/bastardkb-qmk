...
## 2024-05-22 - Add TimeRemainingColumn to progress bar
**Learning:** For long-running CLI tasks (like building dozens of firmwares), showing just the time elapsed is insufficient. Users need to know when the task will complete to manage their time effectively.
**Action:** Always include estimated completion time (e.g. `TimeRemainingColumn` in `rich`) in progress bars for tasks that take more than a few seconds.
