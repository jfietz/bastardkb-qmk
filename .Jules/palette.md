...
## 2024-05-29 - [Wait Anxiety Reduction]
**Learning:** UX guidelines for CLI tools using `rich.progress` recommend including a `TimeRemainingColumn` alongside `TimeElapsedColumn` for long-running batch operations. This provides users with an estimated completion time and reduces wait anxiety during extensive builds.
**Action:** When implementing long-running batch progress bars, always pair `TimeElapsedColumn` with `TimeRemainingColumn` to manage user expectations.
