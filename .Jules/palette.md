...
## 2024-05-29 - [Progress Bar Wait Anxiety]
**Learning:** Long-running batch CLI operations (like building multiple firmwares) can cause user wait anxiety if only elapsed time is shown.
**Action:** Always include a `TimeRemainingColumn` alongside `TimeElapsedColumn` in rich progress bars for batch processes to provide an estimated completion time and improve the user experience.
