...
## 2024-05-29 - [Batch Operation Time Estimates]
**Learning:** Long-running CLI operations (like batch firmware builds) create user wait anxiety when only showing elapsed time. Including a time remaining estimate significantly improves the perception of performance and provides actionable context for the wait.
**Action:** Always include a `TimeRemainingColumn` (or equivalent) alongside `TimeElapsedColumn` in progress bars for operations where the total item count is known upfront.
