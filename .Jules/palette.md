...
## 2024-05-29 - [Batch Process Estimated Completion]
**Learning:** Users experience anxiety during long-running batch processes (like building multiple firmwares) when only elapsed time is shown. Providing an estimated time remaining transforms the progress bar from a simple tracker into a planning tool, allowing users to safely step away.
**Action:** Always include a `TimeRemainingColumn` alongside `TimeElapsedColumn` when using `rich.progress` for batch operations that take more than a few seconds.
