...
## 2024-05-29 - [Progress Estimations]
**Learning:** For long-running batch operations, showing only time elapsed can cause wait anxiety for the user. Adding an estimated time remaining column provides critical context and a better progress tracking UX.
**Action:** When implementing progress bars for tasks that take an unpredictable or long amount of time, include an estimated time remaining alongside time elapsed.
