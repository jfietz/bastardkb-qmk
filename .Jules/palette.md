...
## 2024-05-29 - [Progress Bar Completion Estimates]
**Learning:** Long-running batch operations that only show elapsed time can cause user anxiety and uncertainty. Adding a time remaining estimate provides valuable context and improves the overall CLI user experience.
**Action:** Always include a `TimeRemainingColumn` alongside `TimeElapsedColumn` when using `rich.progress` for batch operations.
