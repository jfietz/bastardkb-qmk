...
## 2024-05-31 - Path Traversal in Log Files
**Vulnerability:** The `Reporter.log_file` method used unsanitized inputs (like git branch names) directly in file paths.
**Learning:** `pathlib.Path` does not prevent path traversal by default, and `with_suffix` can incorrectly truncate stems if the input contains dots.
**Prevention:** Always sanitize directory separators in unpredictable inputs before concatenation, and use string formatting for file extensions instead of `with_suffix`.
