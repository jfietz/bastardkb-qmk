...
## 2026-11-25 - Arbitrary File Read via Symlink Resolution during File Globbing
**Vulnerability:** Insecure file collection during asset copy. The script used `Path.is_file()` to filter files in a potentially untrusted directory before copying them. However, `is_file()` follows symlinks by default. If a symlink was placed in the directory pointing to a sensitive file (e.g., `/etc/passwd`), the script would read the target file and copy its contents to the output directory, leading to an arbitrary file read vulnerability.
**Learning:** Python's `Path.is_file()` follows symlinks, which can be dangerous when operating on directories where users or other processes can create symlinks.
**Prevention:** Always explicitly check for symlinks using `not Path.is_symlink()` when filtering files in untrusted directories, even when using `is_file()`.
