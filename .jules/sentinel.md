...
## 2026-11-20 - Arbitrary File Read Vulnerability via Symlink with Path.is_file
**Vulnerability:** Insecure file collection using `Path.is_file()`. Since `is_file()` follows symlinks by default, it could include malicious symlinks pointing to sensitive files in the application's repository. These files would then be copied to the output directory and exposed.
**Learning:** Python's `Path.is_file()` follows symlinks by default. If untrusted input directories are scanned for files to distribute, this can lead to arbitrary file read vulnerabilities.
**Prevention:** Always verify if the file path is a symlink using `not path.is_symlink()` when collecting files from untrusted or shared directories.
