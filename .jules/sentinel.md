...
## 2024-04-29 - Path traversal and suffix truncation in pathlib
**Vulnerability:** User-controlled input (like git branch names) passed to `Path.with_suffix()` can cause unexpected path traversal or truncation, as pathlib treats parts after dots as extensions.
**Learning:** When sanitizing user inputs for filenames (e.g., branches like `feature/test`), `.with_suffix(".log")` removes anything after the last dot in the sanitized string, resulting in data loss, and lacks protection against path traversal.
**Prevention:** Explicitly sanitize path separators (replace `/` and `\` with `_`) and use simple string concatenation (f-strings) instead of `.with_suffix()` for appending extensions to unpredictable strings.
