...
## 2026-11-20 - Arbitrary File Read Vulnerability via Symlink with Path.glob
**Vulnerability:** Insecure file collection operation with `Path.glob()`. When copying via definition files from an untrusted repository, the script used `[f for f in via_json_dir.glob("*.via.json") if f.is_file()]`. Because `is_file()` follows symlinks by default, an attacker could include malicious symlinks in the repository pointing to sensitive local files (like `~/.ssh/id_rsa`), which would then be copied into the output directory and potentially exposed.
**Learning:** Python's `Path.is_file()` returns `True` for symlinks that point to regular files. When collecting files from untrusted sources, this default behavior can lead to arbitrary file read vulnerabilities if the files are subsequently copied or exposed.
**Prevention:** When traversing directories and acting on files from an untrusted source, always explicitly check `not f.is_symlink()` alongside `f.is_file()` to prevent following symlinks.
