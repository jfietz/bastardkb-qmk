...
## 2026-11-25 - Arbitrary File Read via Symlink Resolution
**Vulnerability:** Arbitrary file read in file collection step (`copy_assets_to_output_dir`). The build script used `Path.glob()` and `Path.is_file()` to find `.via.json` files and copy them. `Path.is_file()` resolves symlinks. A malicious git repository could contain a `.via.json` symlink pointing to sensitive system files (like `/etc/passwd`), which would then be resolved and copied into the final build artifacts directory by the build server, exposing sensitive internal files.
**Learning:** Python's `Path.is_file()` (and similar functions like `os.path.isfile`) follows symbolic links by default. When iterating and operating over files provided in untrusted directories (like checked out git repositories), blindly following symlinks can easily lead to arbitrary file read (or write) vulnerabilities.
**Prevention:** When traversing an untrusted directory tree to collect files, explicitly use `not path.is_symlink()` to ensure you are only interacting with regular files and not following potentially malicious links.
