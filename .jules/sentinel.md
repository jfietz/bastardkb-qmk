...
## 2026-11-21 - Path Traversal in Log Filenames
**Vulnerability:** Path traversal and unhandled `FileNotFoundError` caused by unsanitized git branch names containing directory separators (e.g., `feature/branch` or `../../../etc/passwd`) used in log filenames.
**Learning:** `pathlib.Path.with_suffix()` can truncate stems if the input string contains dots (e.g., `.._.._.._etc_passwd`), removing valid parts of the sanitized input. Replacing directory separators is necessary to prevent directory traversal and crashes when writing to nonexistent paths.
**Prevention:** Sanitize user inputs used in filenames by replacing `/` and `\` with `_`. To safely append file extensions to unpredictable strings containing dots, use f-strings or string concatenation instead of `.with_suffix()`.
