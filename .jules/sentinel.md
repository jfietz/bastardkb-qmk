...
## 2024-05-20 - Path Traversal via Log Filename
**Vulnerability:** Insecure log file path generation in `Reporter.log_file(basename)`. Passing branch names with slashes (e.g., `feature/branch-v1.0`) caused path truncation with `.with_suffix(".log")` and arbitrary directory paths.
**Learning:** `Path.with_suffix()` truncates the stem if the input contains dots (e.g., `.._.._.._etc_passwd` becomes `.._.._..log`). Slashes in user-controlled names like branches can be interpreted as directories.
**Prevention:** Sanitize user input by replacing slashes and backslashes with underscores. Use f-strings or string concatenation for extensions on unpredictable strings instead of `with_suffix()`.
