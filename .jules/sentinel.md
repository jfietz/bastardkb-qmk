...
## 2026-11-20 - Arbitrary File Read via Symlink Collection
**Vulnerability:** The script collected all `.via.json` files for copying to an output directory using `Path.glob()` and `Path.is_file()`. Since `is_file()` resolves symlinks by default, it inadvertently included symlinks pointing to sensitive files outside the repository. Copying these symlinks resulted in the sensitive file contents being written to the output directory (an Arbitrary File Read vulnerability).
**Learning:** `Path.is_file()` returns `True` for symlinks that point to actual files. When processing files from an untrusted source or directory, relying solely on `is_file()` can lead to arbitrary file reads or processing if malicious symlinks are present.
**Prevention:** Always explicitly check for symlinks using `not Path.is_symlink()` in addition to `Path.is_file()` when iterating over or collecting files from directories that could contain untrusted input.
