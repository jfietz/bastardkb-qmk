...
## 2026-05-28 - Path Traversal in Log File Creation
**Vulnerability:** Path traversal and missing directory crashes when creating log files for branch names containing directory separators (e.g., `feature/branch`).
**Learning:** When using user-supplied names like Git branches for filenames, failure to sanitize directory separators (`/` and `\`) leads to unintended directory traversal or failures if the parent directory doesn't exist. Path `with_suffix` will improperly truncate filename stems containing multiple periods after the slash is removed.
**Prevention:** Explicitly sanitize filename inputs by replacing directory separators (`/` and `\`) with a safe character (`_`). Use f-strings for appending extensions if the input string can contain multiple arbitrary dots to prevent extension replacement logic from truncating the filename stem.
