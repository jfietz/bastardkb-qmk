...
## 2026-03-30 - Arbitrary File Read via Symlink with Path.is_file()
**Vulnerability:** Insecure file collection using `Path.is_file()` to find configuration files to copy into build artifacts. Because `is_file()` follows symlinks by default, a local attacker or malicious source directory could include a symlink pointing to sensitive system files (e.g., `/etc/passwd`). The script would then read the target of the symlink and copy its contents into the build output, causing an arbitrary file read vulnerability.
**Learning:** Python's `Path.is_file()` and related methods follow symlinks unless explicitly checked. When copying files from untrusted directories (like `via/`), failing to check if a file is a symlink can lead to sensitive data exposure in the artifacts.
**Prevention:** Always explicitly check `not f.is_symlink()` when gathering files for copy operations, especially from directories where you do not fully control the contents.
