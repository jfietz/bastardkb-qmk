...
## 2024-05-30 - Prevent arbitrary file read via symlink in glob
**Vulnerability:** When scanning for files to copy using `Path.glob()`, `Path.is_file()` was used to filter results. However, `is_file()` follows symlinks by default, meaning a malicious symlink in the source directory could lead to arbitrary files outside the intended directory being read and copied to the output directory.
**Learning:** `Path.is_file()` is not sufficient to prevent following symlinks. It returns True for symlinks that point to regular files.
**Prevention:** Always use `not f.is_symlink()` explicitly when iterating over files from an untrusted source to prevent arbitrary file read vulnerabilities.
