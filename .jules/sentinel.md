...
## 2026-11-25 - Path Traversal via Log File Creation
**Vulnerability:** Path traversal vulnerability due to unsanitized input used as a filename. The `basename` used to construct log file paths was not sanitized, allowing a user to potentially overwrite arbitrary files outside the intended log directory using traversal characters (like `/` or `\`).
**Learning:** Functions accepting user or branch names to create files can easily be manipulated if the input is not explicitly sanitized. Using `Path(dir, name)` alone does not prevent `name` from climbing out of `dir`. Additionally, using `Path.with_suffix()` on strings with dots behaves incorrectly by removing the trailing text (treating it as an extension).
**Prevention:** Explicitly sanitize inputs used in path construction by replacing directory separators (`/`, `\`). Use string formatting instead of `.with_suffix()` when handling arbitrary file names with unpredictable dots.
