...
## 2026-05-06 - Path Traversal in Log File Generation
**Vulnerability:** Unsanitized user inputs (e.g., git branch names) were directly passed to `Path(log_dir, basename)` in `Reporter.log_file()`. This allowed path traversal (e.g., using `../../../` or absolute paths) and could lead to `FileNotFoundError` crashes or writing files outside the designated log directory. Additionally, using `with_suffix(".log")` incorrectly truncated filenames containing multiple dots.
**Learning:** `pathlib.Path` resolves paths with `/` and `\`. If an input begins with `/` or contains traversal sequences, it navigates out of the intended directory. Furthermore, `with_suffix()` replaces the text after the *last* dot, which corrupts strings that natively contain dots but aren't extensions.
**Prevention:** Always sanitize directory separators (replace `/` and `\` with `_`) before concatenating user inputs with paths. To safely append extensions to unpredictable strings, use f-strings or string concatenation instead of `with_suffix()`.
