...
## 2024-06-07 - Fix path traversal vulnerability in Reporter.log_file
**Vulnerability:** Path traversal vulnerability in `Reporter.log_file()` where unsanitized user inputs (like branch names with `/`) could write log files to arbitrary locations or crash due to `FileNotFoundError`.
**Learning:** `pathlib.Path.with_suffix()` incorrectly truncates the stem if the input string contains dots (e.g., `.._.._.._etc_passwd.log` becomes `.._.._..log`).
**Prevention:** Explicitly sanitize inputs by replacing directory separators (`/` and `\`) with underscores (`_`) and use string concatenation or f-strings for appending file extensions to unpredictable strings.
