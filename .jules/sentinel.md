...
## 2026-11-21 - Arbitrary File Read via Symlink Source during Copy
**Vulnerability:** Symlinks in an untrusted source directory were implicitly followed and copied by default file-collection logic `Path.glob().is_file()`, leading to arbitrary file read when the copied content was exposed (e.g., published as a build artifact).
**Learning:** `Path.is_file()` returns `True` for symlinks that point to valid files. When copying untrusted source directories containing user-controlled names or symlinks, this behavior converts an attacker's symlink into a real file at the destination containing the targeted file's content.
**Prevention:** When iterating and copying files from untrusted or shared directories, explicitly check `not Path.is_symlink()` alongside `Path.is_file()` to prevent following symlinks.
