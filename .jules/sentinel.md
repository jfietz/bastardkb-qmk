...
## 2024-05-09 - Path Traversal Vulnerability in log_file
**Vulnerability:** Path traversal and truncation vulnerability in `Reporter.log_file` due to unsanitized slashes (`/` and `\`) in user input and improper use of `with_suffix(".log")` resulting in unsafe file writes outside of the log directory.
**Learning:** Branch names and other user inputs can contain directory separators (like `/` or `\`). These must be sanitized before concatenating with `pathlib.Path`. Furthermore, `with_suffix` truncates filenames containing dots by treating the portion after the last dot as a file extension.
**Prevention:** Explicitly sanitize user inputs by replacing directory separators (`/`, `\`) with a safe character (`_`). To append an extension securely without risking truncation, use f-strings or string concatenation before instantiating the `pathlib.Path` object.
