...
## 2024-05-24 - Fix path traversal in Reporter.log_file creation
**Vulnerability:** The `Reporter.log_file` method used unsanitized user input (`basename`) to construct file paths, allowing path traversal (e.g., via `../../../etc/passwd`). Also, using `.with_suffix(".log")` truncates the stem if the input contains dots.
**Learning:** Never trust input used to construct file paths. `Path(dir, basename)` will blindly resolve path traversals if `basename` contains `../` or `/`. Also, `.with_suffix()` replaces anything after the last dot, so `.._.._.._etc_passwd` becomes `.._.._.._etc.log` rather than appending `.log`.
**Prevention:** Explicitly sanitize path separators by replacing `/` and `\` with `_` before joining paths. When appending an extension to arbitrary strings that might contain dots, use string formatting (`f"{sanitized_basename}.log"`) instead of `.with_suffix()`.
