...
## 2026-11-20 - Arbitrary File Read via Symlink Source during Globbing
**Vulnerability:** Insecure file collection during asset copying using `Path.glob()`. When copying `.via.json` files from an untrusted Git repository, `if f.is_file()` was used to filter the results. However, `is_file()` follows symlinks by default. If a repository contained a malicious symlink ending in `.via.json` pointing to a sensitive system file (e.g., `/etc/passwd`), the script would resolve the symlink and copy the sensitive file's contents into the public release output directory.
**Learning:** In Python, standard path checking functions like `Path.is_file()` and `os.path.isfile()` follow symlinks. When iterating over untrusted directories to collect files, this can lead to arbitrary file read vulnerabilities if the destination of the copy operation is publicly accessible.
**Prevention:** Always verify if a discovered file is a symlink when collecting files from untrusted sources. Use `if f.is_file() and not f.is_symlink():` to explicitly exclude symlinks before performing read or copy operations.
