...
## 2026-11-21 - Path Traversal in Log Paths and pathlib Suffix Truncation
**Vulnerability:** Unsanitized git branch names or user inputs passed to `Path` led to path traversal. Additionally, `pathlib.Path.with_suffix` truncated the stem if the input string contained dots.
**Learning:** Always sanitize user input meant for file paths by replacing directory separators. Never use `with_suffix()` on unpredictable user input that may contain dots, as it incorrectly treats text after the last dot as a file extension.
**Prevention:** Use `basename.replace("/", "_").replace("\\", "_")` for sanitization, and use string concatenation or f-strings to append extensions safely (e.g., `f"{sanitized_basename}.log"`).
