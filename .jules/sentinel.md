...
## 2024-05-29 - Path Traversal Vulnerability in Log File Creation
**Vulnerability:** The `Reporter.log_file()` method directly concatenated unvalidated user input (`basename`) into a file path, allowing path traversal (e.g., `../../../etc/passwd`). Furthermore, using `.with_suffix(".log")` on paths containing dots incorrectly truncated the stem.
**Learning:** Even internal log file generation can be an attack vector if parts of the filename are derived from untrusted input like branch names or configurations. Standard `pathlib` methods like `with_suffix()` may have unexpected behavior with path traversal characters.
**Prevention:** Always sanitize inputs meant for filenames by replacing directory separators (`/`, `\`) with safe characters (e.g., `_`). Use string formatting to safely append extensions to arbitrary strings instead of `with_suffix()`.
