...
## 2026-11-25 - Path Traversal in Log Files and Path.with_suffix Truncation
...
## 2026-11-25 - Arbitrary Permission Modification via os.chmod Symlink Following
**Vulnerability:** In `Reporter.__init__`, `os.chmod` is used to enforce `0o700` permissions on the `app_log_dir` in case the directory already exists. However, `os.chmod` follows symlinks by default. An attacker could pre-create a symlink at the `XDG_STATE_HOME/bastardkb-qmk` path pointing to an arbitrary sensitive directory/file owned by the user. When the script runs, it will blindly change the target's permissions to `0o700`, potentially exposing or modifying sensitive assets.
**Learning:** Python's `os.chmod` resolves and follows symbolic links. Combining it blindly with user-controllable directory paths like `XDG_STATE_HOME` opens up arbitrary permission modification vulnerabilities if symlinks are not explicitly mitigated.
**Prevention:** Always verify that a path is not a symlink (`os.path.islink`) before applying potentially destructive path-level operations like `os.chmod`, or use secure file descriptors directly.
