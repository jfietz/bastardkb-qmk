...
## 2024-05-16 - Path Traversal in Log File Creation
**Vulnerability:** The `Reporter.log_file` method concatenated user-controlled inputs (git branch names) directly into file paths without sanitization, allowing path traversal attacks.
**Learning:** Using `pathlib.Path` does not prevent path traversal if the arguments themselves contain traversal sequences (`../`). Additionally, sanitizing paths containing dots and then using `with_suffix()` can cause `pathlib` to incorrectly interpret the end of the string as a file extension and truncate it.
**Prevention:** Always explicitly sanitize inputs containing directory separators before passing them to file system APIs. When appending extensions to sanitized strings that may contain dots, use string concatenation or f-strings before path construction instead of `with_suffix()`.
