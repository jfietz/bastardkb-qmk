...
## 2024-05-12 - Prevent Path Traversal in Log Files
**Vulnerability:** Path traversal possible via user-controlled `basename` in `Reporter.log_file` due to direct concatenation with `pathlib.Path`.
**Learning:** `pathlib.Path` resolves `../` and `/` characters during concatenation. Furthermore, `with_suffix()` can incorrectly truncate paths containing dots.
**Prevention:** Explicitly sanitize user inputs by replacing directory separators (`/` and `\`) before passing them to `Path` constructors. Use string concatenation for extensions if the base string can contain unpredictable dots.
