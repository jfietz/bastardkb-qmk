...
## 2026-11-25 - Arbitrary File Read via Symlink
**Vulnerability:** When copying asset files using `Path.glob()` and `Path.is_file()` from an untrusted source repository directory into the output directory, symbolic links were not explicitly filtered out. `Path.is_file()` returns `True` for symlinks that point to regular files. This could allow arbitrary file read vulnerabilities if an attacker places a symlink pointing to a sensitive file in the source repository.
**Learning:** Python's `Path.is_file()` follows symlinks by default. If a script blindly processes files matched by `is_file()` from a potentially attacker-controllable source, it may inadvertently read and expose sensitive files located elsewhere on the filesystem via a symlink.
**Prevention:** Always explicitly verify that a file is not a symlink when reading or copying files from an untrusted directory. Use `if f.is_file() and not f.is_symlink():` to prevent arbitrary file read vulnerabilities.
