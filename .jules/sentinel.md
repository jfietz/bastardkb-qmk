...
## 2026-11-20 - Path Traversal Vulnerability via Unsanitized Basename
**Vulnerability:** Path traversal vulnerability in `Reporter.log_file`. The method concatenates `basename` (derived from git branch/worktree names) to a path directory `Path(self.log_dir, basename).with_suffix(".log")` without sanitizing directory separators like `/` and `\`. This allows log files to be created outside the intended log directory.
**Learning:** `Path()` concatenation and `.with_suffix()` do not inherently sanitize or protect against path traversal characters like `..` or slashes (`/`, `\`). Furthermore, `with_suffix(".log")` incorrectly truncates base names containing multiple dots (e.g. `.._etc_passwd`) by interpreting text after the last dot as a file extension.
**Prevention:** Explicitly sanitize string parameters representing file names to replace or remove directory traversal characters before path concatenation. Use string formatting (f-strings) instead of `.with_suffix()` when handling file names that might contain unexpected dots to prevent unintended truncation.
