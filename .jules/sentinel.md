...
## 2024-05-24 - Path Traversal in Log File Creation
**Vulnerability:** Path traversal vulnerability in `Reporter.log_file()` where unsanitized user input (`basename`) was joined with `self.log_dir`. This could allow arbitrary file creation or `FileNotFoundError` crashes outside the designated temporary directory. Furthermore, `pathlib.Path.with_suffix(".log")` truncates everything after the last dot if the basename contains dots, leading to incorrect filenames.
**Learning:** `pathlib.Path` safely resolves `../` if appended via `os.path.join` or `Path()`, meaning an attacker could escape the intended base directory. Also, `with_suffix()` is unsafe for appending extensions to strings with unpredictable dot placements (like sanitized traversal paths).
**Prevention:** Explicitly sanitize directory separators (`/` and `\`) from user-provided inputs before path concatenation. Append extensions via string formatting (e.g., `f"{basename}.log"`) instead of `with_suffix()` when dealing with sanitized user inputs containing dots.
