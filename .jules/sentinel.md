...
## 2026-11-25 - Arbitrary File Read via Symlink Follow with Path.is_file()
**Vulnerability:** The script previously gathered files to copy using `Path.glob()` and verified them with `f.is_file()`. This allows a malicious local user to plant a symlink pointing to a sensitive file. Because `is_file()` follows symlinks by default, the sensitive file's contents would be copied into the output directory, leading to an arbitrary file read vulnerability.
**Learning:** Python's `pathlib.Path.is_file()` follows symlinks. When collecting and reading/copying files from directories that could potentially be modified or influenced by untrusted sources, checking `is_file()` is insufficient to prevent symlink-based arbitrary file reads.
**Prevention:** Always verify that a path is not a symlink when handling files from untrusted or shared directories by explicitly checking `not f.is_symlink()` in addition to `f.is_file()`.
