...
## 2026-11-21 - Arbitrary File Read via Symlink Source with Path.is_file()
**Vulnerability:** Insecure file discovery using `Path.is_file()` during `copy_assets_to_output_dir`. `is_file()` returns `True` for symbolic links pointing to valid files. This allows an attacker who controls the source directory to place a symlink (e.g. `malicious.via.json -> /etc/passwd`), causing the script to copy the targeted sensitive file into the output directory, leading to an arbitrary file read vulnerability.
**Learning:** Python's `pathlib.Path.is_file()` resolves symlinks by default. When iterating through a potentially untrusted directory, relying solely on `is_file()` allows malicious symlinks to be processed as normal files.
**Prevention:** When discovering files to process or copy from an untrusted location, always explicitly check `not f.is_symlink()` in addition to `f.is_file()` to ensure only regular files are operated on.
