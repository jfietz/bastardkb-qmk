...
## 2024-04-25 - Fix path traversal and truncation in log_file
**Vulnerability:** Path traversal vulnerability in `Reporter.log_file` where unsanitized user input (like Git branch names containing `/`) was passed directly to `pathlib.Path`, allowing arbitrary file creation outside the intended log directory. Additionally, `Path.with_suffix(".log")` incorrectly truncated filenames if the branch name contained dots (e.g., `feature.branch.name`).
**Learning:** `pathlib.Path` concatenates paths but does not sanitize directory separators (`/` or `\`). Furthermore, `with_suffix` treats any text after the last dot as a file extension and replaces it, which is unsafe for unpredictable strings.
**Prevention:** Explicitly sanitize inputs used in path construction by replacing directory separators (e.g., `.replace('/', '_').replace('\\', '_')`). To safely append extensions to strings containing arbitrary dots, use f-strings or string concatenation before passing to `Path`.
