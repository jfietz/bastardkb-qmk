...
## 2026-11-25 - Path Traversal via Unsanitized Input and with_suffix
**Vulnerability:** In `Reporter.log_file()`, `basename` inputs (like git branch names) were directly passed to `Path()` without sanitization, leading to path traversal (`/` creating unwanted directories). Furthermore, using `with_suffix(".log")` on path traversals containing dots (like `.._.._.._etc_passwd`) truncated the file name.
**Learning:** `Path.with_suffix()` behaves unexpectedly when inputs contain multiple dots (like `_.._.._etc_passwd`), replacing everything after the last dot. This can unintentionally truncate logs. Additionally, concatenating unsanitized inputs from branching tools into paths directly leads to arbitrary directory creation or path traversal vulnerabilities.
**Prevention:** Always sanitize unsanitized inputs (`.replace("/", "_")`) before passing to `Path`. Additionally, to append extensions to variable content containing multiple dots without truncation, use string formatting (`f"{basename}.log"`) instead of `with_suffix()`.
