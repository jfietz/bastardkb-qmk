...
**Vulnerability:** When collecting files to copy from an untrusted source directory, using `Path.is_file()` without checking for symlinks allowed arbitrary file read. An attacker could place a symlink pointing to a sensitive file (like `/etc/passwd`), which would then be copied into the build artifacts, exposing its contents.
**Learning:** Python's `Path.is_file()` resolves symlinks by default. If the source directory is untrusted, relying on it alone is insecure.
**Prevention:** Always verify `not f.is_symlink()` in addition to `f.is_file()` when iterating over files to process or copy from untrusted sources.
