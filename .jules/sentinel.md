...
## 2024-05-07 - Path Traversal in Log File Creation
**Vulnerability:** The `Reporter.log_file` method concatenated untrusted inputs (like Git branch names) directly into file paths. If a branch name contained `/` or `\`, it could cause a `FileNotFoundError` (directory not found) or allow path traversal attacks via `../` sequences, writing logs to arbitrary locations.
**Learning:** `pathlib.Path.with_suffix()` can also unexpectedly truncate the file stem if the sanitized input contains dots, mistakenly treating part of the input as an extension.
**Prevention:** Always sanitize user inputs (e.g., replacing directory separators with underscores) before path concatenation. Use string concatenation for file extensions on unpredictable strings instead of `with_suffix()` to prevent accidental truncation.
