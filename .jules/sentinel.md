...
## 2026-11-25 - Arbitrary File Read via Symlink with Path.is_file
**Vulnerability:** Insecure file collection logic using `Path.is_file()` during asset copying. If an attacker could place a malicious symbolic link in the repository directory pointing to a sensitive local file, `Path.is_file()` would follow the symlink and return `True`. `shutil.copyfile` would then read and copy the contents of the sensitive file to the output directory, exposing arbitrary file contents.
**Learning:** Python's `pathlib.Path.is_file()` follows symlinks by default. When iterating through untrusted directories or processing user-supplied paths, relying solely on `is_file()` can lead to arbitrary file read vulnerabilities if the path is later used in an I/O operation that also resolves symlinks.
**Prevention:** Always verify if a file is not a symlink when collecting files from untrusted sources before reading or copying them. Use `if f.is_file() and not f.is_symlink():` to explicitly exclude symbolic links.
