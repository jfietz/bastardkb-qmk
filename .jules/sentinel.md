...
## 2026-11-21 - Arbitrary File Read Vulnerability with Path.glob and is_file
**Vulnerability:** Insecure file collection using `Path.glob()` combined with `is_file()`. If `is_file()` is used without explicitly checking `is_symlink()`, it will blindly follow symlinks. This can result in reading arbitrary files outside the intended source directory if a malicious symlink is present.
**Learning:** Python's `Path.is_file()` follows symlinks by default. In contexts where source directories are untrusted (e.g., fetching a remote git repository), an attacker can create symlinks (e.g., `malicious.json -> /etc/passwd`) which will be treated as regular files, potentially exposing sensitive files when they are read or copied to the output.
**Prevention:** Always use `not f.is_symlink()` alongside `f.is_file()` when iterating over potentially untrusted files to ensure symlinks are explicitly ignored, preventing arbitrary file read and leakage vulnerabilities.
