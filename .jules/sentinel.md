...
## 2024-05-14 - Fix path traversal in log files
**Vulnerability:** Path traversal via unsanitized branch/firmware names in `Reporter.log_file` allows writing logs outside the intended directory. Furthermore, `with_suffix(".log")` incorrectly drops filename parts containing dots.
**Learning:** Always sanitize user-influenced path segments by replacing directory separators, and use string concatenation rather than `with_suffix` when appending extensions to unpredictable strings containing dots.
**Prevention:** Explicitly replace `/` and `\` with `_` before passing path fragments to `pathlib.Path`, and format extensions as strings to avoid truncation.
