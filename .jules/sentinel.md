...
## 2024-05-24 - Path Traversal in Reporter log_file
**Vulnerability:** Path traversal vulnerability in `Reporter.log_file` allowed arbitrary file paths to be created outside the designated log directory when provided with a malicious basename containing directory separators.
**Learning:** `pathlib.Path` concatenations do not implicitly sanitize directory separators, and using `.with_suffix()` on strings containing dots can lead to incorrect file extensions if the stem is truncated.
**Prevention:** Always explicitly sanitize user-provided or dynamic strings (e.g., branch names, basenames) by replacing directory separators (`/` and `\`) with a safe character (`_`) before passing them to `pathlib.Path`. Use f-strings for appending extensions to unpredictable strings.
