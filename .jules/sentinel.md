...
## 2024-05-17 - Fix Path Traversal Vulnerability in Reporter.log_file
**Vulnerability:** Path traversal vulnerability due to unsanitized input (`basename` containing branch names with slashes) concatenated to `Path` objects, and incorrect handling of dots using `with_suffix(".log")` resulting in file writes outside the intended log directory or unintended file extensions.
**Learning:** `pathlib.Path.with_suffix()` incorrectly truncates stems when dealing with strings containing dots (like `.._.._etc_passwd`), and raw branch names or user inputs should never be passed directly to `Path` concatenation without directory separator sanitization.
**Prevention:** Explicitly sanitize inputs by replacing `/` and `\` with `_`, and manually append the extension to the raw string using f-strings (`f"{sanitized}.log"`) before passing to `pathlib.Path` to prevent truncation.
