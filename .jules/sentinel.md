...
## 2024-05-19 - Path Traversal & Crash Vulnerability in Log File Generation
**Vulnerability:** The `log_file()` method in `Reporter` accepted unsanitized `basename` strings (which could contain git branch names like `feature/branch` or malicious path traversals like `../../../etc/passwd`). This caused `FileNotFoundError` crashes when generating logs for nested branches and posed a path traversal risk allowing arbitrary `.log` files to be created elsewhere. Additionally, using `Path.with_suffix()` could incorrectly truncate strings with multiple dots.
**Learning:** Even seemingly internal strings like git branch names can contain directory separators (`/` or `\`) that break path concatenations. Using `Path.with_suffix()` on arbitrary strings with dots can truncate data incorrectly.
**Prevention:** Always sanitize directory separators in variables intended to be used as file names within a directory (e.g., replacing `/` and `\` with `_`). Use f-strings or string concatenation for adding file extensions to unpredictable strings containing dots instead of `Path.with_suffix()`.
