...
## 2024-05-15 - Path Traversal in Log File Creation
**Vulnerability:** Path traversal vulnerability in `Reporter.log_file` where an unsanitized `basename` could lead to log files being created outside the intended directory.
**Learning:** `Path(self.log_dir, basename)` evaluates to a path outside `self.log_dir` if `basename` contains absolute paths or `../`. Additionally, `with_suffix(".log")` incorrectly truncates stems containing dots (e.g., resulting in `.._.._..log`).
**Prevention:** Explicitly sanitize user inputs used in filenames by replacing directory separators (`/` and `\`) with safe characters (e.g., `_`). Use f-strings for appending extensions to unpredictable strings.
