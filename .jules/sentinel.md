...
## 2024-05-15 - Path Traversal in Reporter.log_file
**Vulnerability:** The `log_file` method accepts a `basename` (which can contain Git branch names) and directly concatenates it to the `log_dir`. When the branch name contains slashes (e.g., `feature/branch`), it creates a nested path, which fails if the directory doesn't exist. It also exposes a path traversal vulnerability if the input contains `../`.
**Learning:** Even when inputs seem internal or domain-specific (like Git branch names), they can contain directory separators that cause path traversal vulnerabilities or unexpected failures when used in file paths.
**Prevention:** Always sanitize inputs used in file paths by replacing directory separators (`/` and `\`) with a safe character (like `_`) before concatenating them to a base path.
