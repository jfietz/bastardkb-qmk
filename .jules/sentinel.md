...
## 2024-05-22 - Path Traversal in Log File Creation
**Vulnerability:** The `Reporter.log_file()` method accepted an unsanitized `basename` parameter to construct file paths. When combined with inputs that could contain directory separators (like branch names `feature/branch`), it could result in files being written outside the intended log directory or causing `FileNotFoundError`.
**Learning:** `pathlib.Path` concatenations do not inherently protect against path traversal if the injected string contains absolute path prefixes or traversal sequences like `../`. Dynamic inputs that are meant to be strictly file names must be sanitized.
**Prevention:** Always sanitize dynamic inputs by explicitly replacing or stripping directory separators (`/` and `\`) before combining them with base directories using `pathlib.Path`.
