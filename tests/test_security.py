...
class TestSecurity(unittest.TestCase):
    ...
    def test_copy_assets_prevents_arbitrary_file_read_via_symlink(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            sensitive_file = td_path / "sensitive.txt"
            sensitive_file.write_text("secret_passwords")

            # Create a malicious symlink in the via_dir pointing to the sensitive file
            ...
            # The malicious symlink should not be copied to the output directory
            copied_file = out_dir / "malicious.via.json"
            self.assertFalse(copied_file.exists(), "Symlink was incorrectly copied exposing sensitive data")
