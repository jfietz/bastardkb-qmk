...
class TestSecurity(unittest.TestCase):
    ...
    def test_copy_assets_prevents_symlink_read(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            # Create a sensitive target file outside the via directory
            sensitive_file = td_path / "sensitive.txt"
            sensitive_file.write_text("secret_read")

            # Create a symlink in the via directory pointing to the sensitive file
            src_symlink = via_dir / "malicious.via.json"
            os.symlink(sensitive_file, src_symlink)
            ...
            # Assert the malicious symlink was NOT copied to the output directory
            dst = out_dir / "malicious.via.json"
            self.assertFalse(dst.exists(), "Symlink was followed and sensitive file was copied")
