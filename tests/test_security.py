...
class TestSecurity(unittest.TestCase):
    ...
    def test_log_file_sanitization(self):
        ...
        path = reporter.log_file("feature/branch_name\\test")
        self.assertEqual(path.name, "feature_branch_name_test.log")
        self.assertEqual(path.parent, Path("/tmp/mock_log_dir"))

        # Test path traversal characters (should not remove dots, but should sanitize slashes)
        path2 = reporter.log_file("../../../etc/passwd")
        self.assertEqual(path2.name, ".._.._.._etc_passwd.log")
        self.assertEqual(path2.parent, Path("/tmp/mock_log_dir"))
