...
class TestSecurity(unittest.TestCase):
    ...
    def test_copy_assets_prevents_arbitrary_file_read(self):
        with tempfile.TemporaryDirectory() as td:
            td_path = Path(td)
            ...
            # Create a sensitive file outside the expected scope
            sensitive_file = td_path / "sensitive.txt"
            sensitive_file.write_text("secret_data")

            # Create a malicious symlink masquerading as a via.json file
            ...
            legitimate_file.write_text("legit_content")
            ...
            # Assert legitimate file was copied
            self.assertTrue((out_dir / "legit.via.json").exists())

            # Ensure the output dir does not contain the malicious file
            dst = out_dir / "malicious.via.json"
            self.assertFalse(dst.exists(), "Arbitrary file read vulnerability: Symlink source was copied")
