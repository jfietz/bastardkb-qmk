...
class TestSecurity(unittest.TestCase):
    ...
    def test_copy_assets_prevents_arbitrary_file_read_via_symlink(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            # Create a malicious symlink pointing to the sensitive file
            malicious_symlink = via_dir / "malicious.via.json"
            os.symlink(sensitive_file, malicious_symlink)

            # Create a normal file
            normal_file = via_dir / "normal.via.json"
            normal_file.write_text("normal_data")
            ...
            # Assert normal file was copied successfully
            self.assertTrue((out_dir / "normal.via.json").exists())
            self.assertEqual((out_dir / "normal.via.json").read_text(), "normal_data")

            # Assert malicious symlink was NOT copied, preventing arbitrary file read
            self.assertFalse((out_dir / "malicious.via.json").exists(), "Malicious symlink was copied, exposing arbitrary file contents!")
