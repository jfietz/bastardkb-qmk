...
class SecureRotatingFileHandler(RotatingFileHandler):
    """A RotatingFileHandler that creates log files with restricted permissions."""
    def _open(self):
...
def copy_assets_to_output_dir(executor: Executor, reporter: Reporter, output_dir: Path, repository_path: Path):
    ...
    # Explicitly check not f.is_symlink() to prevent arbitrary file read attacks where a malicious
    # repo could contain a symlink to a sensitive file (e.g. /etc/passwd) and get it copied to output.
    via_json_list = [f for f in via_json_dir.glob("*.via.json") if f.is_file() and not f.is_symlink()]
...
def main() -> None:
    ...
    # Parse the filter regex, handling invalid patterns gracefully.
    try:
        filter_regex = re.compile(cmdline_args.filter)
    except re.error as e:
        reporter.fatal(
            f"Invalid filter regular expression provided: [bold]'{cmdline_args.filter}'[/bold]\n\n{str(e).capitalize()}",
            title="Invalid Regex",
        )
        sys.exit(1)
    ...
    build(
        executor,
        reporter,
        apply_filter(ALL_FIRMWARES, filter_regex),
        partial(
